...
public class Provisioner {
  ...
  public static Provisioner from(MeshStorage.Provisioner provisioner) {
    ...
    for (MeshStorage.Provisioner.AddressRange unRange : provisioner.allocatedUnicastRange) {
      pv.allocatedUnicastRange
          .add(new AddressRange(MeshUtils.hexToIntB(unRange.lowAddress), MeshUtils.hexToIntB(unRange.highAddress)));
    }

    for (MeshStorage.Provisioner.AddressRange unRange : provisioner.allocatedGroupRange) {
      pv.allocatedGroupRange
          .add(new AddressRange(MeshUtils.hexToIntB(unRange.lowAddress), MeshUtils.hexToIntB(unRange.highAddress)));
    }

    for (MeshStorage.Provisioner.SceneRange sceneRange : provisioner.allocatedSceneRange) {
      pv.allocatedSceneRange
          .add(new SceneRange(MeshUtils.hexToIntB(sceneRange.firstScene), MeshUtils.hexToIntB(sceneRange.lastScene)));
    }
    return pv;
  }
  ...
  public MeshStorage.Provisioner convert() {
    ...
    for (SceneRange range : allocatedSceneRange) {
      provisioner.allocatedSceneRange.add(
          new MeshStorage.Provisioner.SceneRange(String.format("%04X", range.firstScene),
              String.format("%04X", range.lastScene)));
    }
    return provisioner;
  }
}