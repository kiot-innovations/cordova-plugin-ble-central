...
public class NodeInfo implements Serializable {
   ...
   * can be add by
   * {@link com.telink.ble.mesh.core.message.config.NetKeyAddMessage}
  ...
  private OfflineCheckTask offlineCheckTask = (OfflineCheckTask) () -> {
    ...
    TelinkMeshApplication.getInstance().dispatchEvent(new NodeStatusChangedEvent(TelinkMeshApplication.getInstance(),
        NodeStatusChangedEvent.EVENT_TYPE_NODE_STATUS_CHANGED, NodeInfo.this));
  };
  ...
  public List<Integer> getEleListByModel(int targetModelId) {
    ...
    outer: for (Element element : compositionData.elements) {
  ...
  /**
   * @param associatedModelId target model id
   *                          {@link MeshSigModel#getLevelAssociatedList()}
   * @return element address: -1 err
   */
  public int getLevelAssociatedEleAdr(int associatedModelId) {
    ...
    for (Element element : compositionData.elements) {
      if (element.sigModels != null) {
        if (element.sigModels.contains(associatedModelId)
            && element.sigModels.contains(MeshSigModel.SIG_MD_G_LEVEL_S.modelId)) {
          return eleAdr;
        }
      }
      eleAdr++;
  ...
  public String getPidDesc() {
    ...
    if (bound && compositionData != null) {
      ...
      // pidInfo = (compositionData.cid == 0x0211 ? String.format("%04X",
      // compositionData.pid)
  ...
  /**
   * Triggered only when response is received that sends an unspecified propertyID
   * message
   *
